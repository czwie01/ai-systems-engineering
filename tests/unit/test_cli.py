...
def test_run_ambiguous_completion_succeeds(
    ...
    output = capsys.readouterr().out
    assert "NAIVE RETRY CONTROL" in output
    assert "duplicate effect observed" in output
    assert "IDEMPOTENCY CONTRACT" in output
    assert "REJECTED (idempotency-conflict)" in output
    assert "RETENTION CONTROL" in output
    assert "GUARANTEE" in output
    assert "NON-GUARANTEE" in output
